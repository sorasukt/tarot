...
test("requires an explicit access period when Stripe metadata has none",async()=>{
  ...
  globalThis.fetch=async()=>Response.json({id:"promo_no_plan",code:"FRIEND-CODE2026",active:true,times_redeemed:0,metadata:{}});
  try{
    const result=await syncExistingStripePromotionCode({DB:db(),STRIPE_SECRET_KEY:"test_key"},{promotionCodeId:"promo_no_plan"});
