...
test("billing pages use simple provider-neutral copy and keep membership management in My Account",async()=>{
  ...
  assert.match(membership,/20\/วัน/);
  assert.doesNotMatch(membership,/id="priceComparisonBody"/);
  ...
  assert.match(membershipScript,/Annual Boost/);
  assert.match(membershipScript,/ลงชื่อใช้งาน/);
