...
test('service worker precache is unique, complete for imports, and deletes only own caches',async()=>{
 ...
 const context={self:{addEventListener:(n,f)=>listeners[n]=f,skipWaiting(){},clients:{claim(){}},location:{origin:'https://sorasukt.com'}},caches:{open:async()=>({addAll:async list=>added.push(...list)}),keys:async()=>['another-app','sorasukt-tarot-shell-v1','sorasukt-tarot-shell-v2-audit','sorasukt-tarot-shell-v3-delivery','sorasukt-tarot-shell-v4-home-reference'],delete:async key=>deleted.push(key)},Response,URL};
 ...
 listeners.activate({waitUntil:p=>activated=p});await activated;assert.deepEqual(deleted,['sorasukt-tarot-shell-v1','sorasukt-tarot-shell-v2-audit','sorasukt-tarot-shell-v3-delivery']);
});
