...
test("admin can create Stripe promotion code and receive redeem link",async()=>{
  ...
  try{
    ...
    assert.equal(data.redeem.redeemUrl,"https://sorasukt.com/tarot/redeem/?code=YEAR-TEST2026&plan=yearly");
...
test("admin can list Stripe promotion codes with redeem links",async()=>{
  ...
  try{
    const response=await handleRedeemAdmin(new Request("https://api.sorasukt.com/api/admin/redeem-codes?source=stripe&limit=25"),{DB:db(),STRIPE_SECRET_KEY:"test_key",SITE_URL:"https://sorasukt.com"},headers,session);
    const data=await response.json();
    ...
    assert.equal(data.codes[0].redeemUrl,"https://sorasukt.com/tarot/redeem/?code=MONTH-TEST2026&plan=monthly");
  }finally{globalThis.fetch=originalFetch}
});
